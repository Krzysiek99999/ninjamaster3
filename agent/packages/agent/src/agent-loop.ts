...
async function runLoop(
	...
	if (filesToPrefetch.length > 0) {
		try {
			...
			// F1 (2026-04-27): empirically, on multi-file surgical UI rewrite tasks (e.g. t1
			// where task names 5+ React components: ExpertProfileHero, ExpertProfileSidebar, ...)
			// the v232 pre-deletion exploit BACKFIRES — cursor edits files surgically, our
			// gutting forces over-deletion → denominator inflates → similarity drops. Aceini
			// (no pre-deletion) beat Junjie4 by +17 matched on this task class.
			// Detect: ≥5 distinct PascalCase compound identifiers OR ≥4 named file paths
			// in task text → suppress pre-deletion (keep prefetch+nudge intact).
			const _f1PascalSet = new Set<string>();
			const _f1PascalRe = /\b([A-Z][a-z]+(?:[A-Z][A-Za-z0-9]+)+)\b/g;
			let _f1m: RegExpExecArray | null;
			while ((_f1m = _f1PascalRe.exec(rawTaskText)) !== null) _f1PascalSet.add(_f1m[1]);
			const isMultiFileSurgical = _f1PascalSet.size >= 5 || rawTaskFiles.length >= 4;
			...
			for (const filePath of filesToPrefetch.slice(0, 6)) {
				try {
					...
					if (isVolumeTask && lines.length >= 80 && !isMultiFileSurgical) {
